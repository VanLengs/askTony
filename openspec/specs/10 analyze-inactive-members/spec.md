# analyze-inactive-members Specification

## Purpose
TBD - created by archiving change analyze-inactive-members. Update Purpose after archive.
## Requirements
### Requirement: Analyze inactive members command
系统 SHALL 提供一个新的分析命令 `asktony analyze inactive-members`，用于识别在最近 N 个月内提交次数为 0 的成员，并在终端展示结果或导出为 CSV。

#### Scenario: User runs the command
- **WHEN** 用户执行 `asktony analyze inactive-members`
- **THEN** 系统输出“最近 2 个月提交次数为 0 的成员”列表（默认 `months=2`）

### Requirement: Inactive definition and time window
系统 MUST 支持通过 `--months` 指定时间窗口（最近 N 个月，N>=1）。在该窗口内，成员的 `commit_count` 统计值为 0 时，成员 MUST 被判定为 inactive 并出现在结果中。

#### Scenario: User changes the analysis window
- **WHEN** 用户执行 `asktony analyze inactive-members --months 3`
...
- `commit_count`：统计窗口内提交次数（恒为 `0`）

#### Scenario: User inspects the returned columns
- **WHEN** 用户查看 `asktony analyze inactive-members` 的输出
...
- **WHEN** 用户执行 `asktony analyze inactive-members --months 2 --all --csv /output/inactive_members.csv`
- **THEN** 系统生成 `/output/inactive_members.csv`，并写入表头与数据行（字符串不截断）
...
- **WHEN** 用户执行 `asktony analyze inactive-members --csv /output/inactive_members.csv`
...
- **WHEN** 用户执行 `asktony analyze inactive-members --all`
- **THEN** 系统返回全量 inactive 成员结果（不受 Top 限制）
