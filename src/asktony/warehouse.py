    ...
    def build(self) -> None:
        with self.db.connect() as conn:
            ...
            conn.execute(
                ...
                SELECT
                  -- 优先用 username/email（更容易与 commits 对齐），最后才用 user_id
                  LOWER(COALESCE(username, email, user_id)) AS member_key,
            ...
            conn.execute(
                ...
                SELECT DISTINCT
                  repo_id,
                  LOWER(COALESCE(NULLIF(username,''), NULLIF(email,''), NULLIF(user_id,''))) AS member_key,
            ...
            conn.execute(
                ...
                COPY (
                  SELECT
                    c.repo_id,
                    c.sha AS commit_sha,
                    LOWER(COALESCE(NULLIF(c.author_username,''), NULLIF(c.author_email,''), NULLIF(c.author_id,''))) AS member_key,
                    c.author_username,
                    c.author_email,
                    c.committed_at,
        ...
        with self.db.connect() as conn:
            rows = conn.execute(
                ...
                SELECT
                  ...
                  c.member_key,
                  COALESCE(m.department_level2_name, 'Unassigned') AS department_level2_name,
                  COALESCE(m.department_level3_name, 'Unassigned') AS department_level3_name,
                  COUNT(*) AS commit_count,
                  SUM(c.changed_lines) AS changed_lines
                FROM gold.fact_commit c
        ...
        with self.db.connect() as conn:
            rows = conn.execute(
                ...
                SELECT
                  ...
                  COALESCE(m.username, c.author_username, c.member_key) AS member,
                  c.member_key,
                  COALESCE(m.department_level2_name, 'Unassigned') AS member_department_level2_name,
                  COALESCE(m.department_level3_name, 'Unassigned') AS member_department_level3_name,
                  COUNT(*) AS commit_count,
                  SUM(c.changed_lines) AS changed_lines
                FROM gold.fact_commit c
            ...
            rows = conn.execute(
                ...
                employee_people AS (
                  SELECT
                    ...
                    arg_min(COALESCE(e.employee_id, ''), CASE WHEN e.member_key LIKE 'dummy_%' THEN 1 ELSE 0 END) AS employee_id,
